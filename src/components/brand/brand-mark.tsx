import type { BrandId } from "@/data/products";

/**
 * Official brand marks, shown on the shop's brand filter as a retailer
 * identifying the goods it stocks.
 *
 * Two kinds:
 * - PATH_MARKS: single-path monochrome outlines on a 24x24 grid, filled with
 *   currentColor so they follow the chip's own foreground colour in either
 *   theme and in the selected state (nike, adidas, jordan — icon-style marks).
 * - IMAGE_MARKS: real wordmark logos (hoka, skechers) that only exist as
 *   full-colour artwork, not a clean single-colour glyph. These render on a
 *   small white plate so the brand's actual ink colour stays legible over
 *   the storefront's dark theme.
 */
const PATH_MARKS: Partial<Record<BrandId, string>> = {
  nike:
...
const IMAGE_MARKS: Partial<Record<BrandId, string>> = {
  hoka: "/brand/hoka-logo.webp",
  skechers: "/brand/skechers-logo.webp",
};

export function hasBrandMark(brand: BrandId) {
  return brand in PATH_MARKS || brand in IMAGE_MARKS;
}

/** Lets callers size/style the chip differently for a wordmark logo (wide,
 * needs room to breathe) versus a square icon mark. */
export function brandMarkKind(brand: BrandId): "path" | "image" | null {
  if (brand in PATH_MARKS) return "path";
  if (brand in IMAGE_MARKS) return "image";
  return null;
}
...
export function BrandMark({
  ...
  const image = IMAGE_MARKS[brand];
  if (image) {
    return (
      <span
        className={`flex items-center justify-center rounded bg-white px-1.5 py-1 ${className ?? ""}`}
      >
        {/* eslint-disable-next-line @next/next/no-img-element -- tiny fixed
            wordmark logo, sized purely by CSS; next/image adds no value here */}
